...
public class Solution {
    ...
    public static class ClassWithStatic {
        ...
        public void save(OutputStream outputStream) throws Exception {
            ...
            outputStream.write(("static="+ staticString).getBytes());
            outputStream.write(13);
            outputStream.write(("i="+ i).getBytes());
            outputStream.write(13);
            outputStream.write(("j="+ j).getBytes());
        }

        public void load(InputStream inputStream) throws Exception {
            //implement this method - реализуйте этот метод
            BufferedReader buf = new BufferedReader(new InputStreamReader(inputStream));
            List<String> strings = buf.lines().collect(Collectors.toList());
            for (String string : strings) {
                if (string.startsWith("static")) {
                    String[] split = string.split("=");
                    ClassWithStatic.staticString = split[1];
